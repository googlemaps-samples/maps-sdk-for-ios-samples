...
class AutocompletePushViewController: AutocompleteBaseViewController {
  ...
  override func viewDidLoad() {
    ...
    NSLayoutConstraint.activate([
      showWidgetButton.leadingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.leadingAnchor),
      showWidgetButton.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor),
      showWidgetButton.topAnchor.constraint(
        equalTo: view.safeAreaLayoutGuide.topAnchor, constant: margin),
  ...
  @objc func showAutocompleteWidget() {
    ...
    if let config = autocompleteConfiguration {
      ...
      autocompleteViewController.placeProperties = config.placeProperties.map { $0.rawValue }
    }
